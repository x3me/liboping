...
#if WITH_DEBUG
#define dprintf(...)                                              \
	printf("%s[%4i]: %-20s: ", __FILE__, __LINE__, __FUNCTION__); \
	printf(__VA_ARGS__)
#else
#define dprintf(...) /**/
#endif
...
int ping_setopt(pingobj_t *obj, int option, void *value)
{
	...
	case PING_OPT_AF:
		...
		if ((obj->addrfamily != AF_UNSPEC) && (obj->addrfamily != AF_INET) && (obj->addrfamily != AF_INET6))
		{
			obj->addrfamily = PING_DEF_AF;
			ret = -1;
		}
...
#ifdef AI_CANONNAME
		if ((ai_ptr->ai_canonname != NULL) && (strcmp(ph->hostname, ai_ptr->ai_canonname) != 0))
		{
			char *old_hostname;
