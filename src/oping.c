...
static char const *const hist_symbols_utf8[] = {
	"▁", "▂", "▃", "▄", "▅", "▆", "▇", "█"};
static size_t const hist_symbols_utf8_num = sizeof(hist_symbols_utf8) / sizeof(hist_symbols_utf8[0]);
...
static size_t const hist_symbols_acs_num = sizeof(hist_symbols_acs) / sizeof(hist_symbols_acs[0]);
...
static size_t const hist_colors_num = sizeof(hist_colors_utf8) / sizeof(hist_colors_utf8[0]);
#endif
...
static void clean_history(ping_context_t *ctx) /* {{{ */
{
	...
	/* Sort all RTTs. */
	qsort(ctx->history_by_value, ctx->history_size, sizeof(ctx->history_by_value[0]), compare_double);
...
static double calc_average_latency(ping_context_t *ctx)
{
	...
	double total = 0;
	for (size_t idx = 0; idx < ctx->history_received; idx++)
	{
		total += ctx->history_by_value[idx];
	}

	return total / ctx->history_received;
}
...
static double context_get_packet_loss(const ping_context_t *ctx) /* {{{ */
{
	...
	return (100.0 * (ctx->req_sent - ctx->req_rcvd) / ((double)ctx->req_sent));
...
__attribute__((noreturn)) static void usage_qos_exit(const char *arg, int status) /* {{{ */
{
	if (arg != 0)
...
static int set_opt_send_qos(const char *opt) /* {{{ */
{
	...
	else if ((strncasecmp("af", opt, strlen("af")) == 0) && (strlen(opt) == 4))
	{
		uint8_t dscp;
		uint8_t class = 0;
		uint8_t prec = 0;
	...
	/* - Class Selector (CS) */
	else if ((strncasecmp("cs", opt, strlen("cs")) == 0) && (strlen(opt) == 3))
	{
		uint8_t class;
	...
	else
	{
		...
		if ((errno != 0) || (endptr == opt) || (endptr == NULL) || (*endptr != 0) || (value > 0xff))
			usage_qos_exit(/* arg = */ opt, /* status = */ EXIT_FAILURE);
...
static int read_options(int argc, char **argv) /* {{{ */
{
	...
	while (1)
	{
		...
		case 'P':
		{
			...
			if (isnan(new_percentile) || (new_percentile < 0.1) || (new_percentile > 100.0))
					  ...
					  const struct timeval *tv_end)
{
	...
	if ((tv_end->tv_sec > ts_dest->tv_sec) || ((tv_end->tv_sec == ts_dest->tv_sec) && ((tv_end->tv_usec * 1000) > ts_dest->tv_nsec)))
	{
		ts_dest->tv_sec = 0;
		ts_dest->tv_nsec = 0;
		return;
...
static int update_graph_boxplot(ping_context_t *ctx) /* {{{ */
{
	...
	for (x = 0; x < x_max; x++)
	{
		...
		else /* (x != 0) */
		{
			...
			else if (((ratios[x] >= 0.25) && (ratios[x] <= 0.75)) || ((ratios[x - 1] < 0.75) && (ratios[x] > 0.75)))
			{
				symbol = BOXPLOT_BOX;
				reverse = 1;
			}
...
static int update_graph_histogram(ping_context_t *ctx) /* {{{ */
{
	...
	for (x = 0; x < x_max; x++)
	{
		...
		if (has_colors() == TRUE)
		{
			if ((ratio_this <= threshold_green) || ((ratio_prev < threshold_green) && (ratio_this > threshold_green)))
				color = OPING_GREEN;
			else if ((ratio_this <= threshold_yellow) || ((ratio_prev < threshold_yellow) && (ratio_this > threshold_yellow)))
				color = OPING_YELLOW;
			else
				color = OPING_RED;
...
static int post_loop_hook(pingobj_t *ping) /* {{{ */
{
		 ...
		 iter = ping_iterator_next(iter))
	{
		...
		*/
		...
		char min[10] = "null", median[10] = "null", avg[10] = "null", max[10] = "null";

		if (context->req_rcvd != 0)
		{
			double d_min, d_median, d_max, d_avg;
			d_min = percentile_to_latency(context, 0.0);
			d_median = percentile_to_latency(context, 50.0);
			d_max = percentile_to_latency(context, 100.0);
			d_avg = calc_average_latency(context);

			if (!isnan(d_min))
				snprintf(min, 10, "%.2f", d_min);

			if (!isnan(d_min))
				snprintf(min, 10, "%.2f", d_min);

			if (!isnan(d_median))
				snprintf(median, 10, "%.2f", d_median);

			if (!isnan(d_max))
				snprintf(max, 10, "%.2f", d_max);
...
int main(int argc, char **argv) /* {{{ */
{
	...
	if ((opt_filename != NULL) && (strcmp("-", opt_filename) != 0) && (getuid() != geteuid()))
	{
